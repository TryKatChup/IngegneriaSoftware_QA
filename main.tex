...
\begin{itemize}
	...
	\begin{itemize}
		\item \texttt{Animale} rimane superclasse, e \texttt{Mammifero} eredita da \texttt{Animale}. \texttt{Pippistrello} deriva da \newline \texttt{Mammifero} ed è legato tramite composizione a \texttt{Volatile}
	\end{itemize}
\end{itemize}
Esiste un ulteriore approccio ed è una combinazione dei metodi \textit{Composizione-delega} e \texttt{interfacce}.
\end{solution}
...
\end{minted}
Il codice sopra consente di mostrare un esempio di polimorfismo per inclusione: viene utilizzato l'\textit{Overriding} per ridefinire il metodo \texttt{Fun1(int x)} della superclasse \texttt{A} in \texttt{B}.
...
\begin{itemize}
	\item Si consideri una classe \texttt{BigliettoConcerto} e una classe \texttt{GestoreVenditaBiglietto} che si occupa della vendita di \texttt{BigliettoConcerto}
	\item Vi è una dipendenza da parte di \texttt{GestoreVenditaBiglietto} verso la classe concreta \\\texttt{BigliettoConcerto}, rendendo poco manutenibile il codice.
	\item Se si creassero infatti le classi concrete \texttt{BigliettoNormale} e \texttt{BigliettoVIP}, che estendono \texttt{BigliettoConcerto},
	\item Per risolvere il problema occorre dichiarare \texttt{BigliettoConcerto} come un'interfaccia.
	\item Nel caso in cui si vogliano aggiungere ulteriori tipologie di biglietto basta creare nuove classi concrete, senza modificare il codie di \texttt{GestoreVenditaBiglietto}
\end{itemize}
\end{solution}
...
\end{problem}
\begin{solution}
Il pattern Observer viene utilizzato nel caso in cui un oggetto (detto \textit{subject}) debba notificare un cambiamento del proprio stato interno a uno o più oggetti (detti \textit{observer}).
...
\end{problem}
\begin{solution}
Il pattern flyweight consente la condivisione di oggetti leggeri tra più entità, in modo tale che il loro uso non sia troppo costoso.
...
\end{itemize}
Una corretta condivisione dei flyweight tra i Clienti viene garantita dall'utilizzo di una FlyweightFactory.
