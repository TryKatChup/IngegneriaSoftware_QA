...
\begin{itemize}
	\item Si supponga di avere la classe \texttt{Pipistrello} che eredita sia dalla classe \texttt{Mammifero} che dalla classe \texttt{Volatile}.
	...
	\begin{itemize}
		...
		\item \texttt{Pipistrello} deriva da \texttt{Mammifero} ed è legato tramite composizione a \texttt{Volatile}.
	\end{itemize}
\end{itemize}
...
assegnaPosto(posto);
Console.writeline(posto); // se PostoRistorante è una classe si ha (3,5)
                          // se è una struttura si ha (1,1)

static void assegnaPosto(PostoRistorante posto){
...
static void assegnaPosto(out PostoRistorante posto){
        posto.NumeroTavolo = 3; posto.NumeroPosto = 5;  // Dà un errore di compilazione
	posto = new PostoRistorante(3,5); // È necessario inizializzarlo
}
	\end{minted}
\end{itemize}
\end{solution}
